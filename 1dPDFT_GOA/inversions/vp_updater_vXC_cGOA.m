function [vpnew,totTs] = vp_updater_vXC_cGOA(x,h,N,RCell,vCell,totDens,DensAlpha,S,dSdnAlpha,vp,EpXC,EpH,vpH,eta)
...
vpnew = vp + eta*((1-S)*sum(vXC_frag_fun.*DensAlpha,2)./(totDens+1e-12) + S*vXC_tot_fun - vXC_tot_inv + (dSdnAlpha./(totDens+1e-12))*(EpXC+EpH/Nfrag) - (1-S)*vpH/Nfrag);
end
